...
def import_csv_file(filepath, framework):
    ...
    with open(filepath, 'r', encoding='utf-8') as f:
        # Skip first row (Table 1), read second row as headers
        lines = f.readlines()
        if len(lines) < 2:
            print("  ERROR: File too short")
            return 0, 0
        
        # Parse with proper headers (row 2)
        reader = csv.DictReader(lines[1:])
        
        for row in reader:
            # Skip empty rows
            if not row.get('Proxy') or row.get('Proxy') == 'Proxy':
                continue
            
            value = parse_value(row.get('£ Value', ''))
            
            # Handle different column names between files
            evidence_note = row.get('Evidence Note', '') or row.get('Research Batch', '')
            source_status = row.get('Source Status', '')
